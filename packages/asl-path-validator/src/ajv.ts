...
export const registerAll = (ajv: Ajv.default, config = AslPathValidatorConfig): void => {
  ...
  ajv.addFormat(config.format_names[AslPathContext.REFERENCE_PATH], (path: string): boolean => {
    return validateAdapter(path, AslPathContext.REFERENCE_PATH)
  })

  ajv.addFormat(config.format_names[AslPathContext.PATH], (path: string): boolean => {
    return validateAdapter(path, AslPathContext.PATH)
  })

  ajv.addFormat(config.format_names[AslPathContext.PAYLOAD_TEMPLATE], (path: string): boolean => {
