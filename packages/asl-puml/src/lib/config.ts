...
const ajv = new Ajv.default({ schemas: [config] })

export const toConfig = (
  configFromUsr?: Partial<UserSpecifiedConfig> | null,
  ...
  const configIsValid = ajv.validate('https://github.com/massfords/asl-puml#', _config)
  if (!configIsValid) {
    if (!ajv.errors) {
      return { isValid: false, message: 'invalid config' }
    }
    const message = ajv.errors
      .map((errobj) => `${errobj.instancePath}:${errobj.keyword}:${errobj.message ?? ''}`)
      .join('\n')
    return { isValid: false, message: `invalid config: ${message}` }
  }

  return { isValid: true, config: _config }
}
