...
import { AslStateType, type AslStatesNode, type StateHints, type StateName } from './types.js'
...
export const build_state_map = (
  ...
  state_map.forEach((value, key) => {
    if (value.json.Type === AslStateType.Map) {
      ...
      child_states.forEach((states) => {
        Object.keys(states).forEach((stateName) => {
          ...
          const { stereotype, deadPath } = compute_stereotype(stateName, child_value, config)
          child_value.stereotype = stereotype
          child_value.deadPath = deadPath ?? false
        })
      })
    }
    if (value.json.Type === AslStateType.Parallel) {
      // update the child states
      const result: Array<Record<string, unknown>> | Array<Record<string, unknown>>[] = JSONPath({
        json: definition,
  ...
  state_map.forEach((hints, stateName) => {
    const { stereotype, deadPath } = compute_stereotype(stateName, hints, config)
