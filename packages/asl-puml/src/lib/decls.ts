...
import type { PumlBuilder, StateHints, StateName } from './types.js'
...
export const decls: PumlBuilder = (_definition, state_map, config: Config) => {
  ...
  const emit_decl = (
    ...
    if (isContainer) {
      if (AslStateType.Parallel === hints.json.Type) {
        ;(hints.json.Branches || []).forEach((_, index) => {
          const branchNumber = index + 1
          // add branch open
          accum.lines.push(`state "Branch ${branchNumber}" as state${hints.id}_${branchNumber} {`)
    ...
    if (comment) {
        ...
        .map((pattern) => {
          ...
          return {
            ...
            comment:
              ...
              firstPeriod !== -1 ? comment.substring(0, firstPeriod + 1) : comment,
      ...
      if (noteConfig) {
        const { comment: _comment, commentConfig } = noteConfig
        must(_comment && commentConfig, 'note config missing required fields', {
          _comment,
          commentConfig,
        })
        accum.lines.push(`note ${commentConfig.side}`)
        accum.lines.push(
          wordwrap(truncate(_comment, 512), {
