...
import type { LineConfig } from './generated/config.js'
import { must } from './must.js'
import type { AslChoiceTransitionNode, PumlBuilder, StateHints } from './types.js'
import { AslStateType } from './types.js'
...
export const transitions: PumlBuilder = (definition, state_map, config) => {
  ...
  const emit_transition_with_color = (args: {
    ...
    const deadpathLine: string | null =
      srcHint.deadPath || targetHint?.deadPath ? lineFromStyle(deadpathStyle) : null
    const LHS = `state${srcHint.id}`
    let transitionLine = null
    let RHS = null
    const label = extraHints && extraHints.label ? extraHints.label : ''
    if (targetHint && targetHint.json.Type === AslStateType.Fail) {
    ...
    } else if (targetHint && srcHint.json.Type === AslStateType.Choice) {
      RHS = `state${targetHint.id}`
      transitionLine = '-->'
    } else if (targetHint) {
      RHS = `state${targetHint.id}`
      transitionLine = '-->'
    } else if (!targetHint && srcHint.json.Type === AslStateType.Fail && srcHint.parent === null) {
      RHS = '[*]'
      transitionLine = lineFromStyle(config.theme.lines.toFail)
...
${label}
  ...
  state_map.forEach((hints) => {
    ...
    if (['Task', 'Parallel', 'Map'].indexOf(hints.json.Type) !== -1 && hints.json.Catch) {
      ...
      hints.json.Catch.forEach((katch) => {
        ...
        emit_transition_with_color({
          ...
          extraHints: {
            label: config.theme.excludeCatchComment ? null : katch.Comment ?? null,
            fromCatch: true,
          },
        })
      })
